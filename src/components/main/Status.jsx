...
const Status = (props) => {
  ...
  let rate = useSelector((state) => state.progress);
  ...
  useEffect(() => {
    instance({
      ...
      dispatch({
        type: "load-progress",
        progress: response.data.progressRate,
        date: response.data.startDays,
      });
    });
  }, [rate]);
    ...
    return (
      <div className="container-status">
        <div className="container-status-main">
          <Badge
            pill
            bg={props.friendField === "front" ? "primary" : "success"}
          >
            {props.friendField}
          </Badge>
        </div>
