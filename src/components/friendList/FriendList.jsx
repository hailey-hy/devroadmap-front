...
const FriendList = () => {
  ...
  for (
    let number = 1;
    number <= Math.ceil(recordAdd.length / recordAddPerPage);
    number++
  ) {
    itemsAdd.push(
      <Pagination.Item
        key={number}
        active={number === activeAdd}
        onClick={() => {
  ...
  let randomItems = [];
  if (switchCheck === true) {
    randomItems.push(<RandomFriendFront />, <RandomFriendBack />);
  }
  ...
  return (
    <section id="friend">
          ...
          <div id="friend-util">
            <div className="util" id="util-search">
              ...
              <div id="search-divider">
                ...
                />
                <Button ref={target} id="btn-search" onClick={() => onSearch()}>
                  검색
                </Button>
              </div>
            </div>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <FriendAddList
            record={currentRecordAdd(recordAdd)}
            loading={loadingAdd}
          ></FriendAddList>
